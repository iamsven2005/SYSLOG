/**
 * systemMetrics.ts - 2025-05-25 by sven.tan
 *
 * Provides various functions for fetching, processing, and managing system logs, metrics, and usage data.
 *
 * Functionality:
 * - **Logging Middleware**: Monitors query performance, logs the time taken for each query.
 * - **getLogs**: Fetches logs with customizable filters (search, hosts, actions, rule groups, CPU, memory thresholds) and supports pagination.
 * - **processLoginLogoutPairs**: Identifies and pairs login and logout events based on the host and user.
 * - **getDeviceUsageData**: Retrieves device usage data (CPU and memory) over a specified time range.
 * - **getMemoryUsageData**: Retrieves memory usage data over a specified time range, grouped by timestamp and host.
 * - **getSensorData**: Retrieves sensor data for system metrics (e.g., CPU, temperature) over a specified time range.
 * - **getDiskUsageData**: Retrieves disk metrics data over a specified time range, grouped by host and disk name.
 * - **deleteLog / deleteMultipleLogs**: Deletes one or multiple logs by their ID.
 * - **deleteLogsByTimePeriod**: Deletes logs older than a specified time period (e.g., 1 day, 7 days, 30 days).
 * - **deleteAuthLogsByTimePeriod**: Deletes authentication logs older than a specified time period.
 *
 * Usage:
 * - Use `getLogs` to retrieve logs with dynamic filters for searching and analyzing logs.
 * - Use `getDeviceUsageData`, `getMemoryUsageData`, `getSensorData`, and `getDiskUsageData` to fetch system usage data for monitoring and visualization.
 * - Use `deleteLogsByTimePeriod` and `deleteAuthLogsByTimePeriod` for cleanup of old logs.
 * - Implement the logging middleware to monitor query performance.
 *
 * Limitations:
 * - The query and data fetching logic may need optimization for large datasets.
 * - The system assumes the presence of certain tables (e.g., `logs`, `memory_usage`, `diskmetric`, `system_metrics`) and their structure.
 * - The logs matching mechanism for login/logout pairing is simplistic and may not handle complex cases.
 *
 * Improvements:
 * - Consider adding more fine-grained role-based access control (RBAC) for log deletion operations.
 * - Introduce more sophisticated filtering options (e.g., based on user-defined fields).
 * - Implement error handling for scenarios where no matching logs are found for pairing login/logout events.
 * - Support real-time updates for monitoring metrics using websockets or server-sent events (SSE).
 */
