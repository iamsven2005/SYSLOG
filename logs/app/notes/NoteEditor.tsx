/**
 * NoteEditor.tsx - 2025-05-25 by sven.tan
 *
 * A rich text editor component for creating and editing notes with support for text formatting, links, images, and more.
 * The editor supports drag-and-drop image uploads, file pasting, and the option to insert base64-encoded images.
 *
 * Functionality:
 * - **Title Input**: A text input for entering the title of the note.
 * - **Rich Text Editor**: Provides a variety of rich text formatting options like bold, italic, underline, headings, blockquotes, etc.
 * - **Image Handling**: Allows users to drag-and-drop or paste images into the editor, supporting file size validation.
 * - **Save/Update**: Supports saving a newly created note or updating an existing note with validation for required fields.
 * - **Undo/Redo**: Includes undo and redo functionality for easy text editing.
 * - **Image Preview**: Displays a preview of the image when it’s successfully added to the content.
 * - **Toast Notifications**: Provides feedback through toast notifications when an image is added or if there are any errors (e.g., file size issues).
 *
 * Usage:
 * - Use this component to create and edit notes in any app that requires text-based content with rich formatting capabilities.
 * - Ideal for note-taking apps, blogs, content management systems, or any platform requiring user-generated content with rich text.
 *
 * Limitations:
 * - The current implementation is limited to basic rich-text formatting; additional complex elements like tables or embedded videos are not included.
 * - File size for images is limited to 5MB, which might not be sufficient for high-resolution images.
 * - The editor may not fully support very large documents with many images, potentially impacting performance.
 *
 * Improvements:
 * - Add support for more advanced content types like tables, code snippets, and video embedding.
 * - Implement a more robust image handling system, such as uploading to a cloud service (e.g., AWS S3 or Cloudinary) rather than using base64 encoding.
 * - Include version history or the ability to track changes made to a note for easier rollback or collaborative editing.
 */
