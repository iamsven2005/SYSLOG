/*
 * equipment-actions.ts - 2025-05-26 by sven.tan
 * Description:
 *   Server-side actions for managing equipment records in a construction or operations-focused system.
 *   Supports full lifecycle management including inventory, loans, maintenance, allocation, and reporting.
 *
 * Features:
 *   - Equipment Categories:
 *     - `getEquipmentCategories`, `createEquipmentCategory`
 *   - Equipment CRUD:
 *     - `getEquipment`, `getEquipmentById`, `createEquipment`, `updateEquipment`
 *   - Equipment Loans:
 *     - `createEquipmentLoan`, `returnEquipmentLoan`
 *   - Maintenance Tracking:
 *     - `createEquipmentMaintenance`
 *   - Project Allocations:
 *     - `allocateEquipmentToProject`, `removeEquipmentAllocation`
 *   - Dashboard Stats:
 *     - `getEquipmentStats` provides breakdowns by status, condition, category, loans, and maintenance schedule
 *
 * Dependencies:
 *   - Prisma client (`db`) and enums (`EquipmentStatus`, `EquipmentCondition`, `MaintenanceType`)
 *   - Next.js cache revalidation (`revalidatePath`)
 *
 * Notes:
 *   - All mutations are followed by targeted `revalidatePath()` to maintain UI consistency
 *   - Quantity fields (e.g., available, damaged, repair) are handled with integrity on each transactional change
 *   - Designed for both admin dashboards and integrated project contexts
 */
