/**
 * utils.ts - 2025-05-26 by sven.tan
 *
 * Description:
 *   This module parses HTML content to extract job title information. It includes two methods for extracting data: one for use in the browser
 *   with the `DOMParser` API, and an alternative method for server-side use when `DOMParser` is not available.
 *
 * Key Features:
 *   - `parseHtmlContent`: Parses the HTML content using `DOMParser` to extract job titles and related information from a table.
 *   - `parseHtmlContentAlternative`: A server-side alternative to `parseHtmlContent` using regular expressions to extract job titles.
 *
 * Key Functions:
 *   - `parseHtmlContent`: Uses the browser's `DOMParser` to parse the HTML and extract job titles from the table.
 *   - `parseHtmlContentAlternative`: Uses regular expressions to extract data for environments without `DOMParser`.
 *
 * Example Usage:
 *   ```ts
 *   const htmlContent = "<html>...</html>"; // HTML content to parse
 *   const jobTitles = parseHtmlContent(htmlContent);
 *   console.log(jobTitles);
 *   ```
 *   ```ts
 *   const jobTitles = parseHtmlContentAlternative(htmlContent);
 *   console.log(jobTitles);
 *   ```
 *
 * Notes:
 *   - The `parseHtmlContent` function expects an HTML table with columns including "S/N", "Job Title", "Abbreviation", "Grade", "Seniority Level",
 *     and "Selectable in Staff CV".
 *   - The `parseHtmlContentAlternative` method uses regular expressions to handle HTML parsing without relying on `DOMParser`, useful for server-side code.
 *   - Both functions return an array of `JobTitle` objects, each containing the job title's relevant details.
 */
