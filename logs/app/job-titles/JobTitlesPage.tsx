/**
 * JobTitlesPage.tsx - 2025-05-26 by sven.tan
 *
 * Description:
 *   This page manages job titles, allowing users to upload job titles via HTML file, search, filter, and delete selected
 *   job titles from the list. The page also includes features for exporting the data to a CSV file.
 *
 * Key Features:
 *   - File upload to import job titles from an HTML file.
 *   - Search and filter job titles based on job title text or seniority level.
 *   - Display a paginated table of job titles with checkboxes for bulk actions.
 *   - Allows deleting selected job titles and exporting to CSV.
 *   - Shows loading and error states while performing operations.
 *
 * Key Functions:
 *   - `handleFileUpload`: Handles the file upload and parsing of job titles from the HTML file.
 *   - `handleSelectAll`: Selects or deselects all job titles for bulk actions.
 *   - `handleSelectJob`: Selects or deselects individual job titles.
 *   - `handleDeleteSelected`: Deletes selected job titles.
 *   - `exportToCSV`: Exports the current job titles to a CSV file.
 *   - `useEffect`: Fetches job titles and updates the UI based on search/filter criteria.
 *
 * Example Usage:
 *   ```tsx
 *   <JobTitlesPage />
 ...
 *
 * Notes:
 *   - The `FileUploader` component handles file uploads and parsing job titles.
 *   - The `getJobTitles`, `bulkInsertJobTitles`, and `deleteJobTitles` functions interact with the backend for fetching, 
 *     inserting, and deleting job titles.
 *   - The CSV export includes columns for job title details like "S/N", "Job Title", "Grade", "Seniority Level", and more.
 *   - The pagination and filtering ensure a smooth user experience when dealing with large datasets.
 */
"use client"
