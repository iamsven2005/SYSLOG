/**
 * TeamsTable.tsx - 2025-05-26 by sven.tan
 *
 * Description:
 *   This component displays a table of teams with features for searching, editing, and deleting teams. It shows relevant details for each team, 
 *   including their name, sequence, description, leaders, members, and locations. Users can also search through leaders, members, and locations.
 *   The component includes a delete confirmation dialog to ensure teams are deleted intentionally.
 *
 * Components:
 *   - `Table`, `TableHeader`, `TableBody`, `TableRow`, `TableCell`, `TableHead`: For displaying team data in a tabular format.
 *   - `Button`: Provides actions for editing, deleting, and adding teams.
 *   - `Input`: Used for searching teams and filtering leaders, members, and locations.
 *   - `Dialog`, `DialogContent`, `DialogHeader`, `DialogTitle`, `DialogFooter`: Used to show a confirmation dialog for deleting a team.
 *   - `Badge`: For displaying the team leaders, members, and locations with their respective styles.
 *   - `Trash2`, `Edit`, `Plus`, `MapPin`: Icons for the corresponding actions (edit, delete, add, location).
 *   - `toast`: Used to display success and error notifications.
 * 
 * Behavior:
 *   - Fetches the list of teams from the database and filters the results based on the search term.
 *   - Users can search for teams by name, sequence, description, remarks, and location.
 *   - Provides buttons for editing and deleting teams. Deletion is confirmed through a dialog box.
 *   - Once a team is deleted, the teams list is updated and displayed.
 *
 * Notes:
 *   - This component handles the filtering of leaders, members, and locations for each team dynamically.
 *   - Team data is fetched from the `getTeams` function, and teams can be deleted using the `deleteTeam` function.
 */
...
import { getTeams, deleteTeam } from "@/app/teams/team-actions"
